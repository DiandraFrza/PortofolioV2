...
function CertificateCard({ imgSrc, title, issuer, credentialUrl, aosDelay, orientation }) {
  ...
  return (
    <a href={credentialUrl} target="_blank" rel="noopener noreferrer" className="group block rounded-2xl overflow-hidden neo-card bg-white dark:bg-zinc-800" data-aos="fade-up" data-aos-delay={aosDelay}>
      ...
      <div className="p-5 flex flex-col justify-between min-h-[140px]">
        ...
        <div className="flex justify-between items-center mt-4 pt-3 border-t-2 border-dashed border-zinc-200 dark:border-zinc-700">
          <div className="flex items-center gap-2 text-xs font-bold text-zinc-600 dark:text-zinc-800">
            <FiAward size={16} className="text-[#a855f7] stroke-[2.5]" />
            <span className="line-clamp-1">{issuer}</span>
          </div>
          <div className="w-7 h-7 flex-shrink-0 rounded-full border-2 border-black bg-white text-[#202020] group-hover:bg-[#a855f7] group-hover:text-white flex items-center justify-center transition-all duration-200 shadow-[1.5px_1.5px_0px_rgba(0,0,0,1)]">
