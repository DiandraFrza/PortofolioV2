      ...
      <div className="neo-card p-6 rounded-xl">
        ...
        <form onSubmit={handleAdd} className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
          ...
          <div className="md:col-span-2 flex justify-end">
            <button type="submit" className="neo-btn px-6 py-2 rounded-lg text-sm font-bold">
              Simpan Skill
            </button>
          </div>
        </form>
        ...
        {isLoading ? (
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {skills.length === 0 ? (
                <tr>
                  <td colSpan="3" className="p-4 text-center text-sm font-bold text-zinc-500">
                ...
                skills.map((s) => (
                  <tr key={s.id} className="border-b border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors">
                    ...
                    <td className="p-3 text-sm">{Array.isArray(s.items) ? s.items.join(", ") : "-"}</td>
